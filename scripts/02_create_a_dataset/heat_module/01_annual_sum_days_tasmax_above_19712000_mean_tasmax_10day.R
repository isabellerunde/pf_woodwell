...
out_dir <- paste0("~/mybucket/probable_futures/working/remo_rerun3/")
out_name <- "annual_sum_days_tasmax_above_19712000_mean_tasmax_10day"

# enter information:: input data  
freq <- "daily" # for directories
var_long_name <- "maximum_temperature" # 
var_short_name <- "tasmax"
rcm <- "regcm" # remo2015 or regcm
data_dir_hist <- "~/climate_data/Probable_futures/heat_module/rcm_data/annual_mean_tasmax_10day/annual_by_time_span/"
...
for(d in 5:length(domains)){ # loop through domains
  ...
  for(m in 1:length(models)){ # loop through models
    ...
    in_file_hist <- paste0(data_dir_hist, "annual_mean_tasmax_10day_", dom, "_", rcm, "_", short_model_name, "_19712000_30yrs.nc")
    out_file_hist <- paste0(out_dir, "mean_annual_mean_tasmax_10day_", dom, "_", rcm, "_", short_model_name, "_19712000.nc")
    system(command = paste0("cdo addc,273.15 -timmean ", in_file_hist, " ", out_file_hist) )
