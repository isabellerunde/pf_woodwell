...
out_name <- "annual_max_consecutive_days_wetbulbmax_above_19712000_1in10_wetbulbmax_3day"
...
data_dir_hist <- "~/climate_data/Probable_futures/heat_module/rcm_data/annual_max_wetbulbmax_3day/wetbulbmax_3day_19712000_1in10/"
...
max_rle <- function(x, val) {
  ...
  if (length(len) > 0) max(len) else 0
}
...
for(d in 1:length(domains)){ # loop through domains
  ...
  for(m in 1:length(models)){ # loop through models
    ...
    in_file_hist <- paste0(data_dir_hist, "wetbulbmax_3day_19712000_1in10_", dom, "_", rcm, "_", short_model_name, ".nc")
    ...
    for(t in c(1:length(time_range))){
      ...
      for(yy in 1:length(threshold_years)){
        ...
        system(command = paste0("cdo -P 16 lec,0 -sub ", in_file_hist, " ", in_file, " ", out_file) )
        ...
        for(i in 1:(dim(dat)[1])){
          for(j in 1:(dim(dat)[2])){
            out_dat[i, j] <- max_rle(dat[i,j,], 1) # function defined above to longest run of 1's
            rm(y)
          }
        }
        ...
        var1 <- ncvar_def("days", "days", list(londim, latdim), 1e32, 
                          paste0("annual maximum length of wetbulbmax-heatwave exceeding the historical 1-10 heatwave in a ", thresh, " world"),
