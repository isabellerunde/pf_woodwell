...
out_name <- "tasmax_3day_19712000_1in10"

# enter information:: input data  
data_dir <- paste0("~/climate_data/Probable_futures/heat_module/rcm_data/annual_max_tasmax_3day/annual_by_time_span/")
var_short_name <- "annual_max_tasmax_3day"
...
for(d in 1:length(domains)){ # loop through domains
  ...
  for(m in 1:length(models)){ # loop through models
    ...
    dat <- ncvar_get(nc, "tasmax") # retrieve variable of interest
    ncdf4::nc_close(nc)
    rm(nc)
    ...
    fillvalue <- 1e32
    dname <- paste0("temperature of the 1-10 year 3-day tasmax heat wave in ", time_range)
