...
out_dir <- paste0("~/")
out_name <- "annual_sum_dry_days"
cdo_command <- "cdo timsum"

# enter information:: input data  
rcm <- "remo2015" # remo2015 or regcm
data_dir <- paste0("~/climate_data/Probable_futures/water_module/rcm_data/annual_sum_dry_days/binary_days_pr_lt_1mm/")
...
for(d in 1:length(domains)){ # loop through domains
  ...
  dom <- domains[d]
  file_name <- paste0("+binary_days_pr_lt_1mm_",  dom, "_", rcm, "_*")
  ...
  for(m in 1:length(models)){ # loop through models
    ...
    for(t in c(1:length(time_range))){
      ...
      for(yy in 1:length(threshold_years)){
        ...
        in_file <- c(Filter(function(x) grepl(paste0(threshold_years[yy], ".nc"), x), model_files) )
        print(in_file)
