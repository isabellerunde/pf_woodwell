...
out_dir <- paste0("~/")
out_name <- "annual_sum_wet_days"
cdo_command <- "cdo timsum"

# enter information:: input data  
rcm <- "remo2015" # remo2015 or regcm
data_dir <- paste0("~/climate_data/Probable_futures/water_module/rcm_data/annual_sum_wet_days/binary_days_pr_ge_1mm/")
...
for(d in 1:length(domains)){ # loop through domains
  ...
  dom <- domains[d]
  file_name <- paste0("+binary_days_pr_ge_1mm_",  dom, "_", rcm, "_*")
